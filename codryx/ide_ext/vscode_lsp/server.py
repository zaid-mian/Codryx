...
async def start(host: str, port: int) -> bool:
    ...
    ResponseType = None
    HeadersType = None
    # Choose response strategy based on websockets version
    ...
    def _process_request(path: str, request_headers: Any):
        ...
        if path in ("/", "/index.html"):
            try:
                ...
                html = ""
            if not html:
                html = f"""<!doctype html><html><head><meta charset="utf-8"><title>Codryx IDE Dashboard</title><style>body{{font-family:Arial;margin:24px}}</style></head><body><h1>Codryx IDE Dashboard</h1><p>Status: Running</p><p>LSP endpoint: <code>ws://{host}:{port}</code></p><h3>VS Code Instructions</h3><ol><li>Install the Codryx client/extension.</li><li>Configure endpoint: <code>ws://{host}:{port}</code>.</li><li>Connect to see diagnostics and quick fixes.</li></ol></body></html>"""
